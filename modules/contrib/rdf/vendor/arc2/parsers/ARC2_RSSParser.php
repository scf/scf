...
class ARC2_RSSParser extends ARC2_LegacyXMLParser {
  ...
  function extractChannel($els) {
    $res = array($this->rdf . 'type' => array(array('val' => $this->rss . 'channel', 'type' => 'iri')));
    $res = array_merge($res, $this->extractProps($els, 'channel'));
    return array($res[$this->rss . 'link'][0]['val'] => $res);
  }
  
  function extractItem($els) {
    $res = array($this->rdf . 'type' => array(array('val' => $this->rss . 'item', 'type' => 'iri')));
  ...
  function extractProps($els, $container) {
    ...
    foreach ($els as $info) {
      ...
      if ($k) {
        ...
        if ($k == $this->enc . 'enclosure') {
          ...
          foreach (array('length', 'type') as $attr) {
            if ($attr_v = $this->v($attr, 0, $info['a'])) {
              $sub_res[$this->enc . $attr] = array(array('val' => $attr_v, 'type' => 'literal'));
            }
          }
          $struct[$v] = $sub_res;
        }
        ...
        $res[$k][] = array('val' => $v, 'type' => in_array($k, $this->dt_props) || !preg_match('/^[a-z0-9]+\:[^\s]+$/is', $v) ? 'literal' : 'iri');
      }
    }
    return $res;
  }
