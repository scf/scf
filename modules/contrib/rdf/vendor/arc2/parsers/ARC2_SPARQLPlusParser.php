...
version:  2008-01-07 (Tweak: CONSTRUCT keyword is now optional)
*/
...
class ARC2_SPARQLPlusParser extends ARC2_SPARQLParser {
  ...
  function xResultVar($v) {
    ...
    if ($sub_r && (list($sub_r, $sub_v) = $this->xVar($result_var)) && $sub_r) {
      $result_var = $sub_r['val'];
    }
    /* * or var */
    if ((list($sub_r, $sub_v) = $this->x('\*', $v)) && $sub_r) {
      return array(array('var' => $sub_r['val'], 'aggregate' => $aggregate, 'alias' => $aggregate ? $result_var : ''), $sub_v);
    }
