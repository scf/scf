...
version:  2008-03-03 (Fix: Entities in XMLLiteral CData sections were decoded)
*/
...
class ARC2_RDFXMLParser extends ARC2_RDFParser {
  ...
  function addT($s, $p, $o, $s_type, $o_type, $o_dt = '', $o_lang = '') {
    //echo "-----\nadding $s / $p / $o\n-----\n";
    $t = array('s' => $s, 'p' => $p, 'o' => $o, 's_type' => $s_type, 'o_type' => $o_type, 'o_dt' => $o_dt, 'o_lang' => $o_lang);
    if ($this->skip_dupes) {
      $h = md5(print_r($t, 1));
  ...
  function reify($t, $s, $p, $o, $s_type, $o_type, $o_dt = '', $o_lang = '') {
    $this->addT($t, $this->rdf.'type', $this->rdf.'Statement', 'iri', 'iri');
    $this->addT($t, $this->rdf.'subject', $s, 'iri', $s_type);
    $this->addT($t, $this->rdf.'predicate', $p, 'iri', 'iri');
    $this->addT($t, $this->rdf.'object', $o, 'iri', $o_type, $o_dt, $o_lang);
  }
  
  /*  */
  
  function open($p, $t, $a) {
    //if ($s = $this->getParentS()) echo "\nnode is " . $s['val'] . "\n";
    //if ($s && isset($s['p'])) echo "p is " . $s['p'] . "\n";
  ...
  function close($p, $t) {
    //echo "state is $this->state\n";
    //if (($s = $this->getParentS()) && isset($s['p'])) echo "p is " . $s['p'] . "\n";
    //echo "closing $t\n";
    switch($this->state){
  ...
  function h1Open($t, $a) {
    ...
    if (isset($a[$this->rdf.'ID'])) {
      $s['type'] = 'iri';
      $s['val'] = $this->calcURI('#'.$a[$this->rdf.'ID'], $s['x_base']);
    }
    /* about */
    elseif (isset($a[$this->rdf.'about'])) {
      $s['type'] = 'iri';
      $s['val'] = $this->calcURI($a[$this->rdf.'about'], $s['x_base']);
    }
    /* bnode */
    else {
      $s['type'] = 'bnode';
      if (isset($a[$this->rdf.'nodeID'])) {
        $s['val'] = '_:'.$a[$this->rdf.'nodeID'];
      }
      else {
        $s['val'] = $this->createBnodeID();
      }
    }
    /* sub-node */
    if ($this->state === 4) {
      ...
      if (isset($sup_s['o_is_coll']) && $sup_s['o_is_coll']) {
        $coll = array('val' => $this->createBnodeID(), 'type' => 'bnode', 'is_coll' => true, 'x_base' => $s['x_base'], 'x_lang' => $s['x_lang']);
        $this->addT($sup_s['val'], $sup_s['p'], $coll['val'], $sup_s['type'], $coll['type']);
        $this->addT($coll['val'], $this->rdf . 'first', $s['val'], $coll['type'], $s['type']);
        $this->pushS($coll);
      }
      /* new entry in existing coll */
      elseif (isset($sup_s['is_coll']) && $sup_s['is_coll']) {
        $coll = array('val' => $this->createBnodeID(), 'type' => 'bnode', 'is_coll' => true, 'x_base' => $s['x_base'], 'x_lang' => $s['x_lang']);
        $this->addT($sup_s['val'], $this->rdf . 'rest', $coll['val'], $sup_s['type'], $coll['type']);
        $this->addT($coll['val'], $this->rdf . 'first', $s['val'], $coll['type'], $s['type']);
        $this->pushS($coll);
      }
      /* normal sub-node */
      elseif(isset($sup_s['p']) && $sup_s['p']) {
        $this->addT($sup_s['val'], $sup_s['p'], $s['val'], $sup_s['type'], $s['type']);
      }
    }
    /* typed node */
    if ($t !== $this->rdf.'Description') {
      $this->addT($s['val'], $this->rdf.'type', $t, $s['type'], 'iri');
    }
    /* (additional) typing attr */
    if (isset($a[$this->rdf.'type'])) {
      $this->addT($s['val'], $this->rdf.'type', $a[$this->rdf.'type'], $s['type'], 'iri');
    }
    /* Seq|Bag|Alt */
    ...
    foreach($a as $k => $v) {
      if (((strpos($k, $this->xml) === false) && (strpos($k, $this->rdf) === false)) || preg_match('/(\_[0-9]+|value|Seq|Bag|Alt|Statement|Property|List)$/', $k)) {
        if (strpos($k, ':')) {
          $this->addT($s['val'], $k, $v, $s['type'], 'literal', '', $s['x_lang']);
        }
      }
    }
    $this->pushS($s);
  ...
  function h2Open($t, $a) {
    ...
    $o = array('val' => '', 'type' => '', 'x_base' => $b, 'x_lang' => $l);
    ...
    if (isset($a[$this->rdf.'resource'])) {
      $o['val'] = $this->calcURI($a[$this->rdf.'resource'], $b);
      $o['type'] = 'iri';
      $this->addT($s['val'], $s['p'], $o['val'], $s['type'], $o['type']);
      /* type */
      if (isset($a[$this->rdf.'type'])) {
        $this->addT($o['val'], $this->rdf.'type', $a[$this->rdf.'type'], $o['val'], 'iri');
      }
      /* reification */
      if (isset($s['p_id'])) {
        $this->reify($this->calcURI('#'.$s['p_id'], $b), $s['val'], $s['p'], $o['val'], $s['type'], $o['type']);
        unset($s['p_id']);
      }
      $this->state = 3;
    }
    ...
    foreach($a as $k => $v) {
      if (((strpos($k, $this->xml) === false) && (strpos($k, $this->rdf) === false)) || preg_match('/(\_[0-9]+|value)$/', $k)) {
        if (strpos($k, ':')) {
          ...
          $this->addT($o['val'], $k, $v, $o['type'], 'literal');
          $this->state = 3;
        }
      }
    }
  ...
  function h4Close($t) {/* empty p | pClose after cdata | pClose after collection */
    if ($s = $this->getParentS()) {
      ...
      if (isset($s['is_coll']) && $s['is_coll']) {
        $this->addT($s['val'], $this->rdf . 'rest', $this->rdf . 'nil', $s['type'], 'iri');
        ...
        if (isset($s['p_id']) && $s['p_id']) {
          $this->reify($this->calcURI('#'.$s['p_id'], $b), $s['val'], $s['p'], $sub_s['val'], $s['type'], $sub_s['type']);
        }
        unset($s['p']);
        $this->updateS($s);
      }
      else {
        $dt = isset($s['o_dt']) ? $s['o_dt'] : '';
        $l = isset($s['p_x_lang']) && $s['p_x_lang'] ? $s['p_x_lang'] : (isset($s['x_lang']) ? $s['x_lang'] : '');
        $o = array('type' => 'literal', 'val' => $s['o_cdata']);
        $this->addT($s['val'], $s['p'], $o['val'], $s['type'], $o['type'], $dt, $l);
        /* reification */
        if (isset($s['p_id']) && $s['p_id']) {
          $this->reify($this->calcURI('#'.$s['p_id'], $b), $s['val'], $s['p'], $o['val'], $s['type'], $o['type'], $dt, $l);
        }
        unset($s['o_cdata']);
        unset($s['o_dt']);
        unset($s['p']);
  ...
  function h6Close($t) {
    if ($s = $this->getParentS()) {
      ...
      $level = $s['o_xml_level'];
      if ($level === 0) {/* pClose */
        $this->addT($s['val'], $s['p'], trim($data, ' '), $s['type'], 'literal', $this->rdf.'XMLLiteral', $l);
        unset($s['o_xml_data']);
        $this->state = 2;
      }
      else {
