...
version:  2008-04-03 (Tweak: skipping triple backups for now
                      Tweak: Changed locking approach from "LOCK TABLE" to "GET LOCK")
*/
...
class ARC2_StoreLoadQueryHandler extends ARC2_StoreQueryHandler {
  ...
  function getTermID($val, $type_id, $tbl) {
    ...
    foreach ($sub_tbls as $sub_tbl) {
      ...
      $sql = "SELECT id AS `id`" . $cid_suffix . ", '" . $sub_tbl . "' AS `tbl` FROM " . $this->store->getTablePrefix() . $sub_tbl . " WHERE val = BINARY '" . mysql_real_escape_string($val) . "'";
  ...
  function bufferTripleSQL($t) {
    ...
    $this->sql_buffers[$tbl] .= $sql . "(" . $t['t'] . ", " . $t['s'] . ", " . $t['p'] . ", " . $t['o'] . ", " . $t['o_lang_dt'] . ", '" . mysql_real_escape_string($t['o_comp']) . "', " . $t['s_type'] . ", " . $t['o_type'] . ")";
  }
  ...
  function bufferTripleBackupSQL($tb) {
    ...
    $this->sql_buffers[$tbl] .= $sql . "(" . $tb['t'] . ", '" . mysql_real_escape_string($tb['data']) . "')";
  }
  ...
  function checkSQLBuffers($force_write = 0, $reset_id_buffers = 0, $refresh_lock = 0, $split_tables = 0) {
    ...
    foreach (array('triple', 'g2t', 'id2val', 's2val', 'o2val') as $tbl) {
      ...
      if ($buffer_size && $force_write) {
        $t1 = ARC2::mtime();
        mysql_query($this->sql_buffers[$tbl]);
        /* table error */
        if ($er = mysql_error()) {
