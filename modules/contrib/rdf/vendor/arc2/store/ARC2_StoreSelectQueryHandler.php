...
version:  2008-04-08 (Tweak: Changed temp table approach from "CREATE SELECT / ALTER" to "CREATE (TEMPORARY) / INSERT SELECT"
                      Fix: getLeftJoins method didn't send "LEFT" to getRequiredSubJoinSQL)
*/
...
class ARC2_StoreSelectQueryHandler extends ARC2_StoreQueryHandler {
  ...
  function runQuery($infos) {
    ...
    if (!mysql_query($tmp_sql) && !mysql_query(str_replace('CREATE TEMPORARY', 'CREATE', $tmp_sql))) {
    ...
    if ($er = mysql_error()) return $this->addError($er);
    $r = $this->getFinalQueryResult($q_sql, $vars, $tmp_tbl, $aggregate_vars);
    mysql_query('DROP TABLE IF EXISTS ' . $tmp_tbl);
    return $r;
  }
