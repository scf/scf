			...
			<tr>
				...
				<xsl:for-each select="front/journal-meta">
					...
					<td valign="top">
						...
						<xsl:if test="custom-meta-wrap">
							<span class="gen">Custom metadata</span>
							<br/>
				...
				<xsl:for-each select="front/article-meta">
					<td valign="top">
						...
						<!-- history/date --><!-- Most recent version of DTD recommends using the <permissions> wrapper
               for the copyright data. We handle both cases here. -->
						...
						<xsl:apply-templates
							select="volume | issue | elocation-id | fpage | lpage" mode="front"/>
...
-->
	...
	<xsl:template
		match="body/sec/sec/title | ack/sec/title | app/sec/title
